# Jules DE ALMEIDA - 18/12/2024
# DES Encryption (Simplified to operate on a single 64-bit block at a time)
...
def feistel_function(right, round_key):
    ...
    subbed = s_box_substitution(xored)
    return permute(subbed, P)
...
def s_box_substitution(bits):
    ...
    for i in range(8):
        ...
        subbed_bits.extend([int(x) for x in f"{S_BOX[i][row][col]:04b}"])
    return subbed_bits
...
def des_encrypt(plaintext, key):
    ...
    for round_key in round_keys:
        ...
        left = temp
    encrypted_bits = permute(right + left, FP)
    # Convert the result to hex for readable output
    return ''.join(f"{int(''.join(map(str, encrypted_bits[i:i + 8])), 2):02x}" for i in range(0, len(encrypted_bits), 8))

# Function only used if the user wants a random key
def generate_random_key():
    import random
    return ''.join(f"{random.randint(0, 255):02x}" for _ in range(8))

# Main function to test our simplified DES algorithm
def main():
    while True:
        print("\nData Encryption Standard (DES) simplified")
        print("My DES uses the core of DES encryption, focusing on a single 64-bit block at a time")
        ...
        if choice == '1':
            ...
            while len(plaintext) != 8:
                ...
                plaintext = input("Enter plaintext (8 characters): ")
            use_random_key = input("Do you want to generate a random key? (Y/N): ").lower()
            while use_random_key != 'y' and use_random_key != 'n':
                use_random_key = input("Do you want to generate a random key? (Y/N): ").lower()
            if use_random_key == 'y':
                key = generate_random_key()
                print(f"Generated Key (in hex): {key}")
            elif use_random_key == 'n':
